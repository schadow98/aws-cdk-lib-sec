 ...
 */
export function checkApplicationLogLevel(stackInput, applicationLogLevel) {
    logger.debug("checkApplicationLogLevel" + applicationLogLevel);
    const stack = Stack.of(stackInput);
 ...
 */
export function checkSystemLogLevel(stackInput, systemLogLevel) {
 ...
 */
export function checkLoggingFormat(loggingFormat) {
 ...
 */
export function checkLogGroup(lambdaId, logGroup) {
 ...
 */
export function checkLogRetention(logRetention) {
 ...
 */
export function checkLogRetentionRole(logRetentionRole) {
    ...
    if (!principal) {
        throw new ConfigurationError("logRetentionRole", "logRetentionRole must have an assumedBy principal defined.");
    }
    // Ensure the principal is a ServicePrincipal
    if (!(principal instanceof ServicePrincipal)) {
        throw new ConfigurationError("logRetentionRole", `logRetentionRole must be assumed by a ServicePrincipal, but found ${principal.constructor.name}.`);
    }
    ...
    if (!allowedServices.includes(principal.service)) {
        throw new ConfigurationError("logRetentionRole", `logRetentionRole should only be assumed by the following services: ${allowedServices.join(', ')}. Found: '${principal.service}'.`);
    }
    ...
    attachedManagedPolicies.forEach((policy) => {
        if (!allowedManagedPolicyArns.includes(policy.managedPolicyArn)) {
            throw new ConfigurationError("logRetentionRole", `logRetentionRole has an unauthorized Managed Policy attached: ${policy.managedPolicyArn}.`);
        }
    });
 ...
 */
export function checkLogRetentionOptions(logRetentionOptions) {
