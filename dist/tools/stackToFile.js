...
export function writeStackToFile(stack, outputDir) {
...
export async function praseFileToStack(filePath) {
    filePath = path.join(process.cwd(), filePath);
    if (!fs.existsSync(filePath)) {
        const consoleMsg = `The configFile doesn't exists: ${filePath}`;
...
export async function stackToFile(stackFilePath, outputDir = "./output") {
    const stacks = await praseFileToStack(stackFilePath);
    for (const index in stacks) {
        writeStackToFile(stacks[index], outputDir);
    }
    return null;
}
//# sourceMappingURL=stackToFile.js.map